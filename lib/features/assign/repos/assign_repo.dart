...
class AssignRepository {
  Future<List<AssignListModel>> lookupPatientInfo() async => (await _patientProvider.lookupPatientInfo()).map((e) => AssignListModel.fromJson(e)).toList();

  Future<AvailableHospitalModel> getAvalHospList(String ptId, int bdasSeq) async => await _patientProvider.getAvalHospList(ptId, bdasSeq);
  Future<String> postReqApprove(Map<String, dynamic> map) async => await _patientProvider.postReqApprove(map);
