...
class AssignProvider {
  ...
  Future<AvailableHospitalModel> getAvalHospList(String ptId, int bdasSeq) async =>
    AvailableHospitalModel.fromJson(
        await _api.getAsync('$_privateRoute/hosp-list/$ptId/$bdasSeq')
    );

  Future<dynamic> postReqApprove(Map<String, dynamic> map) async =>
      await _api.postAsync('$_privateRoute/reqconfirm', toJson(map));
